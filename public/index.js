...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите участника');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Измените ФИО участника', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Участник '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Номер проекта:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Название проекта:<br>' + this.tlTime;
    ...
    button.innerHTML = 'Добавить участника';
